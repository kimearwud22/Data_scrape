...
class tokopedia():
    ...
    def scrapJson(self):
        urlJson = 'https://ace.tokopedia.com/search/product/v3?shop_id={}&rows=80&start=0&full_domain=www.tokopedia.com&scheme=https&device=desktop&source=shop_product'.format(
            self.idToko)
...
'''.format(num+1, '-'*40, i['name'],  i['price'], i['url'], i['image_url'], i['category_name']))
            ...
            self.fullProduk += [{'name': i['name'], 'harga':i['price'],
                                 'link':i['url'], 'image':i['image_url'], 'category':i['category_name']}]
        self.saveProduk()
